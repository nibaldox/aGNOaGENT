...
web_agent = Agent(
    ...
    tools=[SerpApiTools(), PythonTools()],
    ...
    4. Apply date filters to restrict results to the actual year (e.g., using tbs parameters in Google).
...
finance_agent = Agent(
    ...
    #model=Gemini(id="gemini-2.5-flash-preview-05-20"),
    model=Ollama(id="qwen3:4b-fp16"),
    tools=[YFinanceTools(stock_price=True, analyst_recommendations=True, company_info=True), PythonTools()],
    instructions=dedent(f"""\
    Current date: {current_date}
...
#)
#agent_team.print_response(
#    message="What's the market outlook and financial performance of AMSA mining companies (ANTO.L) ? and generate report in Spanish",
#    stream=True,
#)
agent_team.print_response(
